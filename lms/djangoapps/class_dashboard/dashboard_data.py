                ...
                for child in unit.get_children():
                    if child.location.category == 'problem':
                        ...
                        if child.location.url() in prob_grade_distrib:
                            ...
                            problem_name = own_metadata(child)['display_name'].encode('utf-8')
...
def get_d3_sequential_open_distrib(course_id):
    ...
    for section in course.get_children():
        ...
        for subsection in section.get_children():
            c_subsection += 1
            subsection_name = own_metadata(subsection)['display_name'].encode('utf-8')
        ...
        if problem in grade_distrib:  # Some problems have no data because students have not tried them yet.
            ...
            for (grade, count_grade) in grade_distrib[problem]['grade_distrib']:
                ...
                tooltip = "{0} {3} - {1} students ({2:.0f}%: {4:.0f}/{5:.0f} questions)".format(
                    ...
                    count_grade,
                    percent,
                    problem_info[problem]['display_name'].encode('utf-8'),
                    grade,
                    max_grade,
                )
